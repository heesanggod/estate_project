...
public class JwtAuthenticationFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        try {
            ...
            // 접근 주체에 권한을 확인
            UserEntity userEntity = userRepository.findByUserId(userId);  //  예상되는 기대 갯수 0~1개 (0 = null)
            if (userEntity == null) {
                filterChain.doFilter(request, response);
                return;
            }
            String role = userEntity.getUserRole();     // 인증 절차

            // 권한 테이블(리스트) 생성
            List<GrantedAuthority> authorities = new ArrayList<>();
            authorities.add(new SimpleGrantedAuthority(role));   // SimpleGrantedAuthority : 단일권한 객체 문자열 전달 -> 문자열의 규칙 존재 role_
